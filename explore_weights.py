...
def explore_category_weights(sess):
    ...
    for cls in classes[:3]:  # Look at first 3 classes
        ...
        weight_patterns = re.findall(r'(\w+[\s\w]*?)\s*[:=]?\s*(\d+(?:\.\d+)?)\s*%', html_content)
        if weight_patterns:
        ...
        for table in all_tables:
            ...
            if 'category' in str(table_class).lower() or 'weight' in str(table_class).lower():
            ...
            for row in rows:
                ...
                if len(cells) >= 7:
                    ...
                    if category not in categories:
                        categories[category] = {'earned': 0, 'total': 0, 'count': 0, 'weights': []}
